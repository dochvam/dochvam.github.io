...
nimbleEcology distributions condense hierarchical model structure into a
single line of code by a process called **marginalization**.
...
numerically optimizing those parameters to maximize the likelihood. This
is how popular MLE packages for ecological models like `unmarked` work
under the hood. In Bayesian statistics, we don't directly optimize the
likelihood, so we can get away with sampling due to magic (WARNING: I am
not )
...
states *that could have any value* in their domain. To calculate the
likelihood, you have to integrate over all possible values of each
random effect.

(A brief note here: I'm honestly not sure what the definition of "latent
state" is. I believe you can usually think of latent states as a type of
random effect.)

The rapid expansion of Bayesian methods in ecology have both obscured
and been spurred by the fact that writing explicit likelihoods that
integrate over random effects / marginalize over latent states. In
Bayesian statistics, it works out that you can sample the random effect
without having to marginalize over it. This is nice in cases like
continuous random effects where integrating would require complicated
approximators, and also in cases like hierarchical modeling where it's
intuitive to see the model structure written out.
